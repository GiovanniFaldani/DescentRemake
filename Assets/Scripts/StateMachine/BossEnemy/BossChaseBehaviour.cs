...
public class BossChaseBehaviour : StateMachineBehaviour
{
    ...
    [SerializeField] float RangedShootingRange = 10;
    [SerializeField] float MeleeShootingRange = 4;
    float ActualShootingRange;
    ...
    override public void OnStateEnter(Animator animator, AnimatorStateInfo stateInfo, int layerIndex)
    {
        ...
        animator.SetInteger("BossAttackType", Random.Range(0, 2));
        if (animator.GetInteger("BossAttackType") == 0)
        {
            ActualShootingRange = MeleeShootingRange;
        }
        else
        {
            ActualShootingRange = RangedShootingRange;
        }

        if (Vector3.Distance(animator.transform.position, Target.transform.position) <= ActualShootingRange)
        {
    ...
    {
        if (ActualShootingRange != RangedShootingRange && ActualShootingRange != MeleeShootingRange)
        {
            animator.SetInteger("BossAttackType", Random.Range(0, 2));
            if (animator.GetInteger("BossAttackType") == 0)
            {
